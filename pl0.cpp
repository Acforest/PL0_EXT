...
int block(int lev, int tx, bool *fsys)
{
	...
	if (tableswitch) /* 输出名字表 */
	{
		...
		printf("number\tkind\ttype\tname\tval/lev\taddr\tsize\n");
		for (i = tx0 + 1; i <= tx; i++)
		{
			switch (table[i].kind)
			{
			case constant:
				...
				break;
			case variable:
				printf("%d\tvar\t%s\t%s\tlev=%d\taddr=%d\n", i, table[i].type, table[i].name, table[i].level, table[i].adr);
				fprintf(fas, "%d\tvar\t%s\t%s\tlev=%d\taddr=%d\n", i, table[i].type, table[i].name, table[i].level, table[i].adr);
				break;
			case procedur:
		...
		else
		{
			if (table[i].kind == variable) // 类型为变量
			{
				...
				else if (sym == pluseq) // +=运算
				{
					getsymdo;
					memcpy(nxtlev, fsys, sizeof(bool) * symnum);
					expressiondo(nxtlev, ptx, lev); /* 处理+=右侧表达式 */
					if (i != 0)						// 如果不出错，i不等于0，i指向当前语句
					{
						gendo(lod, lev - table[i].level, table[i].adr); // 将变量取到栈顶
						gendo(opr, 0, 2);								// 执行加法
						/* expression将执行一系列指令，但最终结果将会保存在栈顶，执行sto命令完成赋值 */
						gendo(sto, lev - table[i].level, table[i].adr);
					}
			...
			if (i != 0)
			{
				...
				else
				{
					...
					if (sym == tosym)
					{
						...
						// nxtlev[stepsym] = true;	// 后跟符step
						// expressiondo(nxtlev, ptx, lev);

						nxtlev[dosym] = true; // 后跟符do
						expressiondo(nxtlev, ptx, lev);

						// 判断循环变量条件，如 for i := a to b step n do S中，判断i是否小于b，如果是则继续循环，否则跳出循环
				...
				if (sym == lparen) // 如果是左括号
				{
					...
					if (sym == stringsym) // 如果左括号后的第一个参数为字符串，则进行格式化输出
					{
						...
						for (int it = 0; it < str_size; it++)
						{
							...
							{
								gendo(lit, 0, format_str[it]); // 将字符入栈
								gendo(opr, 0, 19);			   /* 输出单个字符 */
							}
						}
						if (sym != rparen)
						{
					...
					else // 输出一般的标识符
					{
						do
						{
							...
							expressiondo(nxtlev, ptx, lev); /* 调用表达式处理，此处与read不同，read为给变量赋值 */
							gendo(opr, 0, 17);				/* 变量输出，生成输出指令，输出栈顶的值 */
						} while (sym == comma);
						if (sym != rparen)
						{
							error(33); /* write()中应为完整表达式 */
...
void interpret()
{
	...
	do
	{
		...
		case sto: /* 栈顶的值存到相对当前过程的数据基地址为a的内存 */
			t--;
			s[base(i.l, s, (int)b) + (int)i.a] = s[t];
			break;
		case cal:					/* 调用子过程 */
