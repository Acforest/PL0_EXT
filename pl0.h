...
#define fprecision 6  /* 新增：浮点数的最小精度 */
...
float num;          /* 改进：当前number，可以为整型数或浮点数 */
...
struct tablestruct
{
    ...
    int val;           	/* 整型或浮点型数值，仅const使用 */
    int level;          /* 所处层，仅const不使用 */
    int adr;            /* 地址，仅const不使用 */
    int size;           /* 需要分配的数据区空间, 仅procedure使用 */
    char type;			/* 新增：数值类型，整型数为 'i'，浮点数为'f' */ 
} table[txmax]; /* 名字表 */
...
#define arraydeclarationdo(a, b, c)   if(-1 == chardeclaration(a, b, c)) return -1		// 新增 

void error(int n);
int getsym();
int getch();
void init();
template <typename T> int gen(enum fct x, int y, T z);
...
int arraydeclaration(int* ptx, int lev, int* pdx);	// 新增：array 声明 
int position(char* idt, int tx);
void enter(enum object k, int* ptx, int lev, int* pdx);
int base(int l, float* s, int b);	// 改进：float 
